  ...
  function onClick() {
    ...
    for (let restaurant of getInputField) {
      ...
      let targetRestaurant = resultObj.find(
        (x) => x.name === currentRestaurant.name
      );

      if (targetRestaurant) {
        let allWorkers = targetRestaurant.employees.concat(
          currentRestaurant.employees
        );
      ...
      let totalCash = currentRestaurant.employees.reduce(
        (x, y) => x + y.salary,
        0
      );
      currentRestaurant.averageSalary =
        totalCash / currentRestaurant.employees.length || 0;
    }
